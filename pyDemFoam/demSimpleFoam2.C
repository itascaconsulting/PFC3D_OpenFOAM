...
#include "demSimpleFoam2.H"

demSimpleFoam2::demSimpleFoam2() {
  Info << "creating demSimpleFoam2 object" << nl << endl;
...
demSimpleFoam2::~demSimpleFoam2() {
  Info << "cleaning up demSimpleFoam2" << nl << endl;
  if (ubar_) delete ubar_;
  if (beta_) delete beta_;
}

void demSimpleFoam2::run(double) {
  ...
  while (simple_->loop())
  {
    ...
    {
      // Momentum predictor
      tmp<fvVectorMatrix> UEqn (fvm::div(fvc::interpolate(n)*phi, U) -
                                fvm::laplacian(n*nu, U) -
                                beta*ubar -
                                fvm::Sp(beta, U)
        );
      ...
      //#include "pEqn.H"
      {
        ...
        {
          fvScalarMatrix pEqn (fvm::laplacian(n*n*rAtU(), p) ==
                               fvc::div(fvc::interpolate(n)*phiHbyA));
